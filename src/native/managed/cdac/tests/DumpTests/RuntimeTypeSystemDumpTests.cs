...
public class RuntimeTypeSystemDumpTests : DumpTestBase
{
    ...
    [ConditionalTheory]
    [MemberData(nameof(TestConfigurations))]
    public void RuntimeTypeSystem_IsObjRefAndIsPrimitive_AreConsistent(TestConfiguration config)
    {
        InitializeDumpTest(config);
        IRuntimeTypeSystem rts = Target.Contracts.RuntimeTypeSystem;
        ILoader loader = Target.Contracts.Loader;
        ...
        Assert.False(rts.IsPrimitive(objectHandle));
        Assert.False(rts.IsPrimitive(stringHandle));
        Assert.False(rts.IsPrimitive(objectArrayHandle));
        Assert.True(rts.IsPrimitive(intPtrHandle));
    }
